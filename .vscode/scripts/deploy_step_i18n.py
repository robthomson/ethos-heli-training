...
def main():
    ...
    # Try language JSON in out_dir first, then fall back to repo i18n folder(s)
    json_path = os.path.join(out_dir, "i18n", f"{lang}.json")
    if not os.path.isfile(json_path):
        tgt_name = os.path.basename(out_dir.rstrip(os.sep))
        json_path = os.path.join(git_src, "src", tgt_name, "i18n", f"{lang}.json")
    if not os.path.isfile(json_path):
